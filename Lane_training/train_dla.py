...
def main():
    ...
    heads = {'hm': 5}
    model_dla = models.get_pose_net2(num_layers=34, heads=heads, head_conv=256, down_ratio=4)
    #model_erf = models.ERFNet3(num_class)
    #model = models.FTFERFNet(num_class)   
    input_mean = model_dla.input_mean
    input_std = model_dla.input_std
    ...
    model_dla = torch.nn.DataParallel(model_dla, device_ids=range(args.gpus)).cuda()
    ...
    train_loader = torch.utils.data.DataLoader(
        getattr(ds, args.dataset.replace("CULane", "VOCAug") + 'DataSet')(data_list=args.train_list, transform=torchvision.transforms.Compose([
            tf.GroupRandomScale(size=(0.640, 0.660), interpolation=(cv2.INTER_LINEAR, cv2.INTER_NEAREST)),
    ...
    optimizer = torch.optim.SGD(itertools.chain(model_FTFA.parameters(), model_FTFB.parameters(), model_dla.parameters()), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    if args.evaluate:
        validate(val_loader, model_A, model_B, model_dla, criterion, 0, evaluator)
        #save_checkpoint2(is_best)
        return
    '''    
    load_FTFA = 'trained/86/model_FTFA.pth.tar'
    ...
    load_FTFB = 'trained/86/model_FTFB.pth.tar'
    ...
    load_dla = 'trained/86/model_dla.pth.tar'
    if os.path.isfile(load_dla):
        print(("=> loading checkpoint '{}'".format(load_dla)))
        load_filename = load_dla
        checkpoint = torch.load(load_filename)        
        model_dla.load_state_dict(checkpoint)            
    '''
    ...
    for epoch in range(args.epochs):  # args.start_epoch
        ...
        # train for one epoch
        train(train_loader, train_loader2, model_A, model_B, model_FTFA, model_FTFB, model_dla, criterion, criterion_exist, criterion_con, optimizer, epoch)
        is_best = 1
        save_checkpoint2(model_A.state_dict(), model_B.state_dict(), model_FTFA.state_dict(), model_FTFB.state_dict(), model_dla.state_dict(), is_best)
...
def train(train_loader, train_loader2, model_A, model_B, model_FTFA, model_FTFB, model_dla, criterion, criterion_exist, criterion_con, optimizer, epoch):
    ...
    for i, (input, target, target_exist) in enumerate(train_loader):
        ...
        output_up, output_exist_up = model_dla(output_up)  # output_mid
        output_down, output_exist_down = model_dla(output_down)  # output_mid
        ...
        if (i + 1) % args.print_freq == 0:
            print((
                    'Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t' 'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t' 'Data {data_time.val:.3f} ({data_time.avg:.3f})\t' 'Loss_up {loss_up.val:.4f} ({loss_up.avg:.4f})\t' 'Loss_down {loss_down.val:.4f} ({loss_down.avg:.4f})\t' 'Loss_exist_up {loss_exist_up.val:.4f} ({loss_exist_up.avg:.4f})\t' 'Loss_exist_down {loss_exist_down.val:.4f} ({loss_exist_down.avg:.4f})\t'.format(
                         epoch+1, i+1, len(train_loader), batch_time=batch_time, data_time=data_time, loss_up=losses_up, loss_down=losses_down, 
    ...
    for i, (input, target, target_exist) in enumerate(train_loader2):
        ...
        if (i + 1) % args.print_freq == 0:
            print((
                    'Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t' 'Time {batch_time.val:.3f} ({batch_time.avg:.3f})\t' 'Data {data_time.val:.3f} ({data_time.avg:.3f})\t' 'Loss_up {loss_up.val:.4f} ({loss_up.avg:.4f})\t' 'Loss_down {loss_down.val:.4f} ({loss_down.avg:.4f})\t' 'Loss_exist_up {loss_exist_up.val:.4f} ({loss_exist_up.avg:.4f})\t' 'Loss_exist_down {loss_exist_down.val:.4f} ({loss_exist_down.avg:.4f})\t'.format(
                         epoch+1, i+1, len(train_loader2), batch_time=batch_time, data_time=data_time, loss_up=losses_up, loss_down=losses_down,
                         loss_exist_up=losses_exist_up, loss_exist_down=losses_exist_down, lr=optimizer.param_groups[-1]['lr'])))
            batch_time.reset()
            data_time.reset()
            losses_up.reset()
...
def save_checkpoint2(state_A, state_B, state_FTFA, state_FTFB, state_dla, is_best):
    if not os.path.exists('trained'):
        os.makedirs('trained')
    #filename = os.path.join('trained', '_'.join((args.snapshot_pref, args.method.lower(), filename)))
    ...
    filename_dla =  os.path.join('trained', 'model_dla.pth.tar')
    #torch.save(state, filename)
    ...
    torch.save(state_dla, filename_dla)
