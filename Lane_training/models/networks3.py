...
class FTF(nn.Module):
    def __init__(self, input_nc, output_nc, path, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        mult = 2
        self.layer2 = nn.Sequential(nn.ConvTranspose2d(ngf * mult, int(ngf * mult),
                                                       kernel_size=3, stride=2,
                                                       padding=1, output_padding=1,
                                                       bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult)),
                                    nn.LeakyReLU(0.2, True))
        ...
        mult = 2
        self.layer5 = nn.Sequential(nn.ConvTranspose2d(ngf * mult, int(ngf * mult),
                                                       kernel_size=3, stride=2,
                                                       padding=1, output_padding=1,
                                                       bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult)),
