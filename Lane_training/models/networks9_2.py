...
'''
def define_GA(num_class, init_type='normal', init_gain=0.02, gpu_ids=[]):

    net = RG(3,3,'A')

    return init_net(net, init_type, init_gain, gpu_ids)

def define_GB(num_class, init_type='normal', init_gain=0.02, gpu_ids=[]):
...
class ResnetGenerator2(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        self.model7 = nn.Sequential(RG.model[25],
                               RG.model[26],
                               RG.model[27]) 
        self.theta1 = nn.Parameter(torch.Tensor(1))
        self.theta2 = nn.Parameter(torch.Tensor(1))
    ...
    def forward(self, input):
        ...
        output3 = self.model3(output2)
        output4 = self.model4(output3)
        alpha = torch.randn(output4.size()).cuda() * self.theta1
        beta = torch.randn(output4.size()).cuda() * self.theta2
        output4 = (1+alpha)*output4 + beta  
        output5 = self.model5(output4)
        alpha2 = torch.randn(output5.size()).cuda()* self.theta1
        beta2 = torch.randn(output5.size()).cuda() * self.theta2
        output5 = (1+alpha2)*output5 + beta2            
        output6 = self.model6(output5)
        alpha3 = torch.randn(output6.size()).cuda() * self.theta1
        beta3 = torch.randn(output6.size()).cuda() * self.theta2
        output6 = (1+alpha3)*output6 + beta3
        output = self.model7(output6)
        
        return output
        
class ResG2(nn.Module):
    def __init__(self, input_nc, output_nc, path, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):

        assert(n_blocks >= 0)
        super(ResG2, self).__init__()
        ...
        self.model1 = nn.Sequential(RG.model1)
        self.model2 = nn.Sequential(RG.model2)
        self.model3 = nn.Sequential(RG.model3)
        self.model4 = nn.Sequential(RG.model4)
        self.model5 = nn.Sequential(RG.model5)
        self.model6 = nn.Sequential(RG.model6)
...
class FTF(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        mult = 1
        self.layer1 = nn.Sequential(nn.Conv2d(ngf * mult, int(ngf * mult / 4), kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult / 4)),
                                    nn.LeakyReLU(0.2, True))
        
        mult = 2
        self.layer2 = nn.Sequential(nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 8),
                                                       kernel_size=3, stride=2,
                                                       padding=1, output_padding=1,
                                                       bias=use_bias),
        ...
        mult = 2
        self.layer5 = nn.Sequential(nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 8),
                                                       kernel_size=3, stride=2,
                                                       padding=1, output_padding=1,
                                                       bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult / 8)),
        ...
        mult = 1
        self.layer6 = nn.Sequential(nn.Conv2d(ngf * mult, int(ngf * mult / 4), kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult / 4)),
                                    nn.LeakyReLU(0.2, True))
        '''
        mult = 2
    ...
    def forward(self, input, input2, input3, input4, input5, input6):
    
        outputf1 = self.layer1(input)
        outputf2 = self.layer2(input2)
        outputf3 = self.layer3(input3)
        output_up = torch.cat([outputf1, outputf2, outputf3], 1)
      
        outputf4 = self.layer4(input4)
        outputf5 = self.layer5(input5)
        outputf6 = self.layer6(input6)
...
class ERFNet3(nn.Module):
    def __init__(self, num_classes, encoder=None):  # use encoder to pass pretrained encoder
        ...
        #self.FTF = FTF(3,3)
        self.initial_block = DownsamplerBlock2(16, 16)
