...
class FTF(nn.Module):
    def __init__(self, input_nc, output_nc, path, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        mult = 1
        self.layer1 = nn.Sequential(nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(ngf * mult * 2),
                                    nn.LeakyReLU(0.2, True))
        
        mult = 2
        self.layer2 = nn.Sequential(nn.Conv2d(ngf * mult, ngf * mult, kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(ngf * mult),
                                    nn.LeakyReLU(0.2, True))
        
        mult = 4
        self.layer3 = nn.Sequential(nn.Conv2d(ngf * mult, int(ngf * mult / 2), kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d( int(ngf * mult / 2)),
                                    nn.LeakyReLU(0.2, True))
        
        self.layer4 = nn.Sequential(nn.Conv2d(ngf * mult, int(ngf * mult / 2), kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d( int(ngf * mult / 2)),
                                    nn.LeakyReLU(0.2, True))
        
        mult = 2
        self.layer5 = nn.Sequential(nn.Conv2d(ngf * mult, ngf * mult, kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(ngf * mult),
                                    nn.LeakyReLU(0.2, True))
         
        mult = 1
        self.layer6 = nn.Sequential(nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(ngf * mult * 2),
                                    nn.LeakyReLU(0.2, True))
        
        mult = 2
        self.layer7 = nn.Sequential(nn.Conv2d(ngf * mult*2, 3, kernel_size=1, stride=1, padding=0, bias=use_bias),
                                    nn.BatchNorm2d(3),
                                    nn.LeakyReLU(0.2, True))
                                    
        self.layer8 = nn.Sequential(nn.Conv2d(ngf * mult*2, 4, kernel_size=1, stride=1, padding=0, bias=use_bias),
                                    nn.BatchNorm2d(4),
                                    nn.LeakyReLU(0.2, True))
                                    
        self.layer9 = nn.Sequential(nn.Conv2d(ngf * mult*2, 16, kernel_size=1, stride=1, padding=0, bias=use_bias),
                                    nn.BatchNorm2d(16),
                                    nn.LeakyReLU(0.2, True))                            
    ...
    def forward(self, input):
        ...
        return outputf16, outputf25, outputf34
...
class Encoder3(nn.Module):
    def __init__(self, num_classes):
        ...
        self.initial_block = DownsamplerBlock(3, 12)
        self.initial_block2 = DownsamplerBlock(16, 48)
    ...
    def forward(self, input, input2, input3, predict=False):
        output = self.initial_block(input)
        output = torch.cat([output, input2], 1)
        output = self.initial_block2(output)
        output = torch.cat([output, input3], 1)
...
class ERFNet3(nn.Module):
    ...
    def forward(self, input, only_encode=False):
        ...
        return self.decoder.forward(output), self.lane_exist(output)
'''
class ERFNet4(nn.Module):
    def __init__(self, num_classes, encoder=None):  # use encoder to pass pretrained encoder
        super().__init__()
       
        self.encoder = Encoder3(num_classes)
        self.decoder = Decoder(num_classes)
        self.lane_exist =  Lane_exist(4)  # num_output
    ...
    def forward(self, input, input2, input3, only_encode=False):
        
        #output_GAN, output_lane = self.FTF(input)
        output = self.encoder(input, input2, input3)   # predict=False by default
        #return output_GAN, self.decoder.forward(output), self.lane_exist(output)
        return self.decoder.forward(output), self.lane_exist(output)
