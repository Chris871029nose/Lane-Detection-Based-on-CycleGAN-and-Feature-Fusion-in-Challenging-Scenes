...
def main():
    ...
    model_A = models.EffG(3,3,'A')
    model_B = models.EffG(3,3,'B')
    ...
    optimizer = torch.optim.SGD(itertools.chain(model_FTFA.parameters(), model_FTFB.parameters(), model_erf.parameters()), args.lr, momentum=args.momentum, weight_decay=args.weight_decay)
    ...
    if args.evaluate:
        validate(val_loader, model_A, model_B, model_erf, criterion, 0, evaluator)
    ...
    for epoch in range(args.epochs):  # args.start_epoch
        ...
        # train for one epoch
        train(train_loader, train_loader2, model_A, model_B, model_FTFA, model_FTFB, model_erf, criterion, criterion_exist, criterion_con, optimizer, epoch)
        is_best = 1
        save_checkpoint2(model_A.state_dict(), model_B.state_dict(), model_FTFA.state_dict(), model_FTFB.state_dict(), model_erf.state_dict(), is_best)
...
def train(train_loader, train_loader2, model_A, model_B, model_FTFA, model_FTFB, model_erf, criterion, criterion_exist, criterion_con, optimizer, epoch):
...
def save_checkpoint2(state_A, state_B, state_FTFA, state_FTFB, state_erf, is_best):
    if not os.path.exists('trained'):
        os.makedirs('trained')
    #filename = os.path.join('trained', '_'.join((args.snapshot_pref, args.method.lower(), filename)))
    ...
    filename_erf =  os.path.join('trained', 'model_erf.pth.tar')
    #torch.save(state, filename)
    ...
    torch.save(state_erf, filename_erf)
