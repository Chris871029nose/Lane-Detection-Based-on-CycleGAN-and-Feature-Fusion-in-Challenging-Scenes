...
class FTF(nn.Module):
    def __init__(self, input_nc, output_nc, path, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        '''
        mult = 1
        self.layer1 = nn.Sequential(nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(ngf * mult * 2),
                                    nn.LeakyReLU(0.2, True))
        
        mult = 2
        self.layer2 = nn.Sequential(nn.Conv2d(ngf * mult, ngf * mult, kernel_size=1, stride=1, padding=0, bias=use_bias),
                                    nn.BatchNorm2d(ngf * mult),
                                    nn.LeakyReLU(0.2, True))
        
        mult = 4
        self.layer3 = nn.Sequential(nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                                       kernel_size=3, stride=2,
                                                       padding=1, output_padding=1,
                                                       bias=use_bias),
        ...
        self.layer4 = nn.Sequential(nn.ConvTranspose2d(ngf * mult, int(ngf * mult / 2),
                                                       kernel_size=3, stride=2,
                                                       padding=1, output_padding=1,
                                                       bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult / 2)),
        ...
        mult = 2
        self.layer5 = nn.Sequential(nn.Conv2d(ngf * mult, ngf * mult, kernel_size=1, stride=1, padding=0, bias=use_bias),
                                    nn.BatchNorm2d(ngf * mult),
                                    nn.LeakyReLU(0.2, True))
         
        mult = 1
        self.layer6 = nn.Sequential(nn.Conv2d(ngf * mult, ngf * mult * 2, kernel_size=3, stride=2, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(ngf * mult * 2),
                                    nn.LeakyReLU(0.2, True))
    ...
    def forward(self, input):
        ...
        return self.layer10(output_up), self.layer10(output_down)
        ...
        #return self.model7(output6), self.layer10(output_up), self.layer10(output_down)
