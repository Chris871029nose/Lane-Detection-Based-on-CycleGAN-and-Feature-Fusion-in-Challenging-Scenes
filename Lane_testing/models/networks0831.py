...
class FTF(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        #state_dict = torch.load('models/latest_net_G_A.pth', map_location=torch.device('cpu'))
    ...
    def forward(self, input):
        ...
        return self.model7(output6), self.layer10(output)
...
class FTFERFNet(nn.Module):
    def __init__(self, num_classes, encoder=None):  # use encoder to pass pretrained encoder
        ...
        #self.encoder = nn.Sequential(erfnet.module.encoder.layers, erfnet.module.encoder.output_conv)
        #self.encoder = nn.Sequential(erfnet.module.encoder.layers)
        #self.encoder = Encoder(num_classes)
