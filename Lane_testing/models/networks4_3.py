...
class ResG(nn.Module):
    ...
    def forward(self, input):
        ...
        return output1, output2, output5, output6 
...
class FTF(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        self.layer10 = nn.Sequential(nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1, bias=use_bias),
                                     nn.BatchNorm2d(16),
                                     nn.LeakyReLU(0.2, True))
        self.layer11 = nn.Sequential(nn.Conv2d(32, 16, kernel_size=3, stride=1, padding=1, bias=use_bias),
                                     nn.BatchNorm2d(16),
                                     nn.LeakyReLU(0.2, True))
    
    def forward(self, input, input2, input5, input6):
    
        outputf1 = self.layer1(input)
        outputf2 = self.layer2(input2)
        #outputf3 = self.layer3(input3)
        output_up = torch.cat([outputf1, outputf2], 1)
      
        #outputf4 = self.layer4(input4)
        outputf5 = self.layer5(input5)
        outputf6 = self.layer6(input6)
        output_down = torch.cat([outputf5, outputf6], 1)
