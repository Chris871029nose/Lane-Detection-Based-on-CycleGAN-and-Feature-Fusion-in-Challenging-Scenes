...
class ResG_or(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):

        assert(n_blocks >= 0)
        super(ResG_or, self).__init__()
        if type(norm_layer) == functools.partial:
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
...
class Encoder2(nn.Module):
    def __init__(self, num_classes):
        ...
        '''
        self.encoder = nn.Sequential(erfnet.encoder.layers[0],erfnet.encoder.layers[1],
                                     erfnet.encoder.layers[2],erfnet.encoder.layers[3],
                                     erfnet.encoder.layers[4],erfnet.encoder.layers[5],
                                     erfnet.encoder.layers[6],erfnet.encoder.layers[7],
                                     erfnet.encoder.layers[8],erfnet.encoder.layers[9],
                                     erfnet.encoder.layers[10],erfnet.encoder.layers[11],
                                     erfnet.encoder.layers[12],erfnet.encoder.layers[13],
                                     erfnet.encoder.layers[14])
...
class ERFNet3(nn.Module):
    def __init__(self, num_classes, encoder=None):  # use encoder to pass pretrained encoder
        ...
        self.decoder = nn.Sequential(erfnet.decoder)
        self.lane_exist = nn.Sequential(erfnet.lane_exist)
