...
def conv_bn(inp, oup, stride):
    return nn.Sequential(
        ...
        nn.Conv2d(inp, oup, 7, stride, 0, bias=False),
        nn.BatchNorm2d(oup),
        nn.ReLU6(inplace=True)
    )
    
def Dconv_bn(inp, oup, stride):
    return nn.Sequential(
        nn.ReflectionPad2d(3),
        nn.Conv2d(inp, oup, 7, stride, 0, bias=False),
        #nn.BatchNorm2d(oup),
...
class DInvertedResidual(nn.Module):
    def __init__(self, inp, oup, stride, expand_ratio):
        super(DInvertedResidual, self).__init__()
        self.stride = stride
        ...
        if expand_ratio == 1:
            self.conv = nn.Sequential(
                # dw
                nn.ConvTranspose2d(hidden_dim, hidden_dim, 3, stride, 1,1, groups=hidden_dim, bias=False),
                nn.BatchNorm2d(hidden_dim),
                nn.ReLU6(inplace=True),
                # pw-linear
                #nn.ConvTranspose2d(hidden_dim, oup, 1, 1, 1, bias=False),
...
class ResnetGenerator2(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        self.features.append(Dconv_bn(32, 3, 1))
...
class ResG(nn.Module):
    def __init__(self, input_nc, output_nc, path, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        '''
        self.model1 = RG.features[0]
        self.model2 = RG.features[1]
        self.model3 = RG.features[2]
        self.model4 = nn.Sequential(RG.features[3],
                                    RG.features[4],
                                    RG.features[5],
                                    RG.features[6],
                                    RG.features[7],
                                    RG.features[8])
        self.model5 = RG.features[9]
        self.model6 = RG.features[10]
        '''
        ...
        self.model1 = nn.Sequential(RG.features[0],
                               RG.model[1],
                               RG.model[2],
                               RG.model[3])
...
class FTF(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.InstanceNorm2d, use_dropout=False, n_blocks=9, padding_type='reflect'):
        ...
        mult = 1
        self.layer1 = nn.Sequential(nn.Conv2d(32, int(ngf * mult / 4), kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult / 4)),
                                    nn.LeakyReLU(0.2, True))
        
        mult = 2
        self.layer2 = nn.Sequential(nn.ConvTranspose2d(64, int(ngf * mult / 8),
                                                       kernel_size=3, stride=2,
                                                       padding=1, output_padding=1,
                                                       bias=use_bias),
        ...
        mult = 4
        self.layer3 = nn.Sequential(nn.ConvTranspose2d(128, int(ngf * mult / 16),
        ...
        self.layer4 = nn.Sequential(nn.ConvTranspose2d(128, int(ngf * mult / 16),
                                                       kernel_size=3, stride=4,
                                                       padding=0, output_padding=1,
                                                       bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult / 16)),
        ...
        mult = 2
        self.layer5 = nn.Sequential(nn.ConvTranspose2d(64, int(ngf * mult / 8),
                                                       kernel_size=3, stride=2,
                                                       padding=1, output_padding=1,
                                                       bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult / 8)),
        ...
        mult = 1
        self.layer6 = nn.Sequential(nn.Conv2d(32, int(ngf * mult / 4), kernel_size=3, stride=1, padding=1, bias=use_bias),
                                    nn.BatchNorm2d(int(ngf * mult / 4)),
                                    nn.LeakyReLU(0.2, True))
        '''
        mult = 2
    ...
    def forward(self, input, input2, input3, input4, input5, input6):
        ...
        #print(input.size(),input2.size(),input3.size(),input4.size(),input5.size(),input6.size())
